...
vector<int> lazy_greedy(vector<set<int>> sets, int k) {
	int korig = k;
	vector<int> res(1 + korig);
	set<int> sol;
	...
	while (k > 0) {
		...
		if (gain(sol, sets[i]) == cost) {
			for (int x : sets[i]) sol.insert(x);
			k--;
			res[korig - k] = sol.size();
		} else {
			cost = gain(sol, sets[i]);
			q.push(make_pair(cost, i));
		}
	}
	for (int i = 1; i <= korig; i++) res[i] = max(res[i], res[i-1]);

	return res;
}

vector<set<int>> window_greedy_smooth(vector<set<int>> sets, int k, int alpha = 10) {
	vector<set<int>> sol(k+1);
	set<int> hist;
...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < nruns; i++) {
		...
		auto sol_lazy = lazy_greedy(sets, K);
		for (int k = 1; k <= K; k++) {
			lazy[k] += sol_lazy[k];
		...
		auto sol = window_greedy_smooth(sets, K);
		for (int k = 1; k <= K; k++) {
			window[k] += sol[k].size();
		}
	}
